    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Subtly warp the Z plane based on position and speed (dampened wave animation)
          p.z = (sin(p.x * 3.0 + uTime) * 0.15 + cos(p.y * 1.5 + uTime) * 0.15) * (0.02 + uSpeed * 0.1);
  ...
  update() {
    ...
    if (this.medias) {
      ...
      this.medias.forEach((media, idx) => {
        ...
        if (dist < minDistance) {
          minDistance = dist;
          closestMediaIndex = idx;
        }
      });

      if (this.onActiveIndexChange && this.originalLength > 0) {
        const activeIdx = closestMediaIndex % this.originalLength;
        if (activeIdx !== this.lastActiveIndex) {
          this.lastActiveIndex = activeIdx;
          this.onActiveIndexChange(activeIdx);
        }
      }
    }
